...
async def student_login(data: StudentLoginInput):
    """Login with student unique code. Auto-registers if code doesn't exist (backward compat)."""
    db = get_db()
    try:
...
async def teacher_request_access(data: TeacherAccessRequestInput):
    """Teacher requests access. Admin must approve before login works."""
    db = get_db()
    try:
...
async def register_teacher(data: TeacherRegisterInput, admin: dict = Depends(require_admin)):
    """Admin creates a new teacher (auto-approved)."""
    db = get_db()
    try:
        existing = db.query(User).filter(User.email.ilike(data.email)).first()
        if existing:
        ...
        hashed = bcrypt.hashpw(data.password.encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
        user = User(name=data.name, email=data.email, password_hash=hashed, role="teacher")
        db.add(user)
        db.commit()
        db.refresh(user)
        return _user_response(user)
