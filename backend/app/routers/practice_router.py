...
async def analyze_craa_speech(
    audio: UploadFile = File(...),
    exercise_id: int = Form(...),
    current_user: dict = Depends(get_current_user),
):
    """Analyze student speech for a CRAA exercise using the CRAA rubric."""
    if current_user["role"] not in ("student", "admin", "teacher"):
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    try:
        ...
        pronunciation_issues = detect_pronunciation_issues(transcript, word_timestamps, None)
        pronunciation_score = calculate_pronunciation_score(transcript, None, pronunciation_issues)

        prosody_data = prosody.dict()
        pron_issues_data = [i.dict() for i in pronunciation_issues]

        logger.info(f"[craa-analyze] Generating CRAA LLM feedback...")
        ...
        # Extract scores from CRAA feedback
        summary_score = craa_feedback.get("summary_accuracy", {}).get("score", 60)
        counter_score = craa_feedback.get("counterargument_quality", {}).get("score", 60)
        delivery_score = craa_feedback.get("verbal_delivery", {}).get("score", 60)
        overall_score = craa_feedback.get("overall_score", round(summary_score * 0.4 + counter_score * 0.3 + delivery_score * 0.3))
        ...
        result = {
            ...
            "scores": {
                "overall": overall_score,
                "summary_accuracy": summary_score,
                "counterargument": counter_score,
                "verbal_delivery": delivery_score,
                "fluency": fluency_eval["fluency_score"],
                "pronunciation": pronunciation_score,
            },
            "craa_feedback": craa_feedback,
            "overall_grade": craa_feedback.get("overall_grade", ""),
            "errors": craa_feedback.get("grammar_errors", []),
        ...
        practice_result = PracticeResult(
            session_id=session.id,
            overall_score=overall_score,
            grammar_score=summary_score,       # repurpose: summary_accuracy
            fluency_score=counter_score,        # repurpose: counterargument
            pronunciation_score=delivery_score, # repurpose: verbal_delivery
            prosody_score=fluency_eval["prosody_score"],
            speech_rate=prosody.speech_rate,
            pause_count=prosody.pause_count,
            mean_pause_duration=prosody.mean_pause_duration,
        ...
        cleanup_files(upload_path, wav_path)
        logger.info(f"[craa-analyze] CRAA analysis complete, session_id={session.id}")
        ...
        logger.error(f"[craa-analyze] CRAA analysis failed: {str(e)}")
        logger.error(traceback.format_exc())
        cleanup_files(upload_path, wav_path)
