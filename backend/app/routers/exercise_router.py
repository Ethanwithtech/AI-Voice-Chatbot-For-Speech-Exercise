...
def _to_response(ex: Exercise, teacher_name: str = None) -> ExerciseResponse:
...
async def list_exercises(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for ex in exercises:
            ...
            result.append(_to_response(ex, teacher.name if teacher else None))
        return result
    finally:
        db.close()
...
async def get_exercise(exercise_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        teacher = db.query(User).filter(User.id == ex.teacher_id).first()
        return _to_response(ex, teacher.name if teacher else None)
    finally:
        db.close()
...
async def create_exercise(data: CreateExerciseInput, teacher: dict = Depends(require_teacher)):
    ...
    try:
        ...
        db.commit()
        db.refresh(ex)
        return _to_response(ex, teacher["name"])
    finally:
        db.close()
...
async def upload_argument_audio(
    ...
    """Upload argument audio for a CRAA exercise."""
    db = get_db()
    try:
        ex = db.query(Exercise).filter(Exercise.id == exercise_id).first()
        if not ex:
        ...
        content_type = (audio.content_type or "").split(";")[0].strip() or "audio/mpeg"
        ex.argument_audio_data = file_bytes
        ex.argument_audio_type = content_type
        db.commit()
        return {"message": "Argument audio uploaded successfully"}
    finally:
        db.close()
...
async def get_argument_audio(exercise_id: int, current_user: dict = Depends(get_current_user)):
    """Stream argument audio for a CRAA exercise."""
    db = get_db()
    try:
        ex = db.query(Exercise).filter(Exercise.id == exercise_id).first()
        if not ex:
        ...
        content_type = ex.argument_audio_type or "audio/mpeg"
        return Response(
            content=ex.argument_audio_data,
            media_type=content_type,
            headers={"Content-Disposition": f"inline; filename=argument_{exercise_id}.mp3"},
        )
    finally:
        db.close()
